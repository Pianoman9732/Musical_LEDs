/**
 * The majority of this code is shamelessly copied from the examples included with the minim sound library. It has
 * been modified to change the color of the frequency bars based on height.
 *
...
public int calcRed(float n) {
  //float n = map(value, 0, range, 0,reference);
  if(n > 3 && n <= 8) {
    return 255;
  }
  else if(n > 10) {
...
public int calcGreen(float n) {
  ...
  if(n > 6 && n <= 8) {
    ...
    //println((int) ((n-12) * (-255/2)));
    return (int) (255 + (n-12) * (-255/2));
  }
  //return 0;
}

public int calcBlue(float n) {
  //float n = map(value, 0, range, 0, reference);
  if(n > 3 && n <= 5) {
    return (int) (255 + (n-3) * (-255/2));
  } else if(n > 10 && n <= 12) {
    return (int) ((n-10) * (255/2));
  } else if(n > 5 && n <= 10) {
    return 0;
  } else return 255;
}